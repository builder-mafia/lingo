...
curl -fsSL https://raw.githubusercontent.com/builder-mafia/lingo/main/install.sh | \
  sh -s -- --version v0.6.0 --install-dir "$HOME/bin"
```
...
노트가 쌓이면 작업공간의 `목록 · 지도` 전환에서 직접 연결한 노트와 코스의 인접 장을 함께 탐색할 수 있습니다. 지도는 관계를 임의로 추론하지 않으며, 모바일에서는 같은 정보를 읽기 쉬운 관계 목록으로 보여줍니다.
