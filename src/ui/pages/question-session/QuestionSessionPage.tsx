  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <Link to={routePaths.notes}>노트</Link><span aria-hidden="true">/</span>
        <Link to={routePaths.note(session.noteId)}>{session.noteTitle}</Link><span aria-hidden="true">/</span>
        <span>질문</span>
      </header>
      ...
      <div className={styles.layout}>
        <main className={styles.main}>
          <div className={styles.questionLabel}>
            ...
            {session.resolvedAt ? "현재 시점에서 정리됨" : session.feedback ? "피드백이 도착했습니다" : session.answer ? "피드백 기다리는 중" : "내 말로 설명해보기"}
          </div>
          <h1>{session.question}</h1>

          {editing && !session.resolvedAt ? (
            <form className={styles.answerForm} onSubmit={submitAnswer}>
              {session.answer ? <p>이전 답변을 바탕으로 다시 정리해보세요. 저장하면 이전 피드백은 새 평가를 위해 비워집니다.</p> : null}
              <label htmlFor="answer">답변</label>
              <textarea id="answer" value={content} onChange={(event) => setContent(event.currentTarget.value)} placeholder="지금 이해한 내용을 자신의 말로 적어보세요." rows={10} autoFocus />
              <div className={styles.actions}>
                {session.answer ? <button className={styles.secondaryButton} type="button" onClick={() => { setEditing(false); setContent(session.answer ?? ""); }}>취소</button> : null}
                <button className={styles.primaryButton} type="submit" disabled={!content.trim() || submitting}>{submitting ? "저장 중…" : "답변 저장"}</button>
              </div>
            </form>
          ) : null}
          ...
          {!editing && session.answer && !session.feedback && !session.resolvedAt ? (
            <section className={styles.waiting}>
              ...
              <p>답변은 저장되었습니다. 외부 AI 스킬이 평가를 기록하면 이 화면과 노트 목록에 표시됩니다.</p>
            </section>
          ) : null}
          ...
          {!editing && session.feedback ? (
            <section className={styles.feedback} aria-labelledby="feedback-heading">
              ...
              {!session.resolvedAt ? (
                <div className={styles.actions}>
                  <button className={styles.secondaryButton} type="button" onClick={() => setEditing(true)}>다시 답하기</button>
                  <button className={styles.primaryButton} type="button" onClick={closeQuestion} disabled={submitting}>이 질문은 여기까지</button>
                </div>
              ) : null}
            </section>
          ) : null}
          ...
          {session.resolvedAt ? (
            <div className={styles.closedActions}>
              ...
              <button type="button" onClick={reopen} disabled={submitting}>질문 다시 열기</button>
            </div>
          ) : null}
