...
test("stores and updates an evaluation for a subjective problem", async () => {
  ...
  try {
    ...
    const problem = JSON.parse((await runCli(home, ["problem", "add", note.noteId, "--data", JSON.stringify({ question: "설명", referenceAnswer: "모범" })])).stdout).data;
    await runCli(home, ["answer", "set", problem.problemId, "--data", JSON.stringify({ content: "답변" })]);
