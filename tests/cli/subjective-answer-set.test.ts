...
test("stores and updates a subjective answer", async () => {
  ...
  try {
    ...
    const problem = JSON.parse((await runCli(home, ["note", "problem", "subjective", "add", note.noteId, "--data", JSON.stringify({ question: "설명하세요", referenceAnswer: "모범 답안" })])).stdout).data;
    const first = await runCli(home, ["answer", "subjective", "set", problem.problemId, "--data", JSON.stringify({ content: "첫 답변" })]);
    const second = await runCli(home, ["answer", "subjective", "set", problem.problemId, "--data", JSON.stringify({ content: "갱신 답변" })]);
