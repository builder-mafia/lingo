...
describe("database migrations", () => {
  ...
  test("adds ordered cascading sources with unique URLs per note", () => {
    const database = new SqliteDatabase(":memory:");
    const noteId = "25aa185c-76c3-48ca-8e98-4b984242defc";
    ...
    try {
      ...
      database
        .query("INSERT INTO notes (id, title, created_at) VALUES (?, ?, ?)")
        .run(noteId, "출처가 있는 노트", "2026-08-01T00:00:00.000Z");
      database
        .query(
          "INSERT INTO note_sources (id, note_id, title, url, description, position, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
        .run(
          crypto.randomUUID(),
          noteId,
      ...
      expect(
        database
          .query<{ readonly count: number }, []>(
            "SELECT COUNT(*) AS count FROM note_sources",
          )
          .get(),
      ).toEqual({ count: 0 });
    } finally {
